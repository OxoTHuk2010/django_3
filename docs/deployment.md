...
Nginx кэширует `/static/` как публичные файлы. Чтобы браузеры не использовали старые CSS и JS после деплоя, базовый шаблон подключает основные ассеты с query-параметром `v`.

В production значение `v` берётся из `STATIC_ASSET_VERSION`, который workflow заполняет текущим `GITHUB_SHA`. При каждом новом deploy URL основных CSS и JS меняется, поэтому браузер запрашивает свежую версию файлов.
