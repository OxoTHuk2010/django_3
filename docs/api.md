...
- REST API централизован в `apps/api`, доменные приложения остаются владельцами моделей и сервисов.
- Product API использует `slug` как основной публичный lookup и поддерживает compatibility route по `id`.
- API-корзина требует JWT и работает только с DB-cart авторизованного пользователя.
- API создаёт заказ только из текущей API-корзины пользователя.
- API-регистрация создаёт пользователя и сразу возвращает JWT pair.
- Review API использует `slug` товара и доменный service-layer отзывов.
- Собственные endpoints проекта используют единый JSON-формат ошибок.
