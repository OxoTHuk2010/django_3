...
- После появления конфликта `C035` always-success mock outcome больше не должен считаться целевым поведением.
- Этап 25 должен уточнить contract для `succeeded`, `failed`, `cancelled` и `pending` payment outcomes.
- До реализации payment emulator подробные примеры ниже считаются историческим контекстом, а не финальным payment contract.
