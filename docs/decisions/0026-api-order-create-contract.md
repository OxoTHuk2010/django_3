...
- API checkout создаёт заказ из текущей DB-cart авторизованного JWT-пользователя.
- API checkout должен использовать тот же orders/cart service layer, что и web checkout.
- Создание заказа остаётся атомарным и повторно проверяет остатки.
- ADR `0033-payment-emulator.md` уточнил payment contract для `succeeded`, `failed`, `cancelled` и `pending`.
- API checkout использует `apps.payment_emulator`: корзина очищается только при `succeeded`, а при `failed`, `cancelled` и `pending` сохраняется.
- Подробные примеры ниже с always-success mock считаются историческим контекстом, а не финальным payment contract.
...
- создаётся ли платёж так же, как в web-сценарии.
...
- создаётся Payment по результату `apps.payment_emulator`;
- после успешной оплаты корзина очищается;
...
7. Service-layer атомарно создаёт Order, OrderItem и Payment.
8. После успешной оплаты корзина очищается.
...
- создаются OrderItem;
- создаётся Payment по результату `apps.payment_emulator`;
- заказ получает статус согласно web checkout;
- корзина пользователя очищается только при `succeeded`;
...
Если Payment в текущей модели имеет другие поля, response serializer должен отражать фактическую модель, но контракт должен явно показывать:
...
После ADR 0033 API использует то же payment emulator поведение, что и web checkout.

Правило:

POST /api/orders/ создаёт заказ и платёж в рамках одной checkout-операции.

API не создаёт отдельный внешний payment flow.

Если в будущем появится настоящий payment provider, это должно быть оформлено отдельным ADR или обновлением ADR 0033.
...
- payment outcome поведение единообразно.
...
9. После создания заказа создаётся Payment по результату `apps.payment_emulator`.
10. После успешной оплаты корзина очищается.
...
6. POST /api/orders/ создаёт Payment.
7. После успешной оплаты DB-cart очищается.
