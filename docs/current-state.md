...
- этапы 20, 21, 22, 23, 24 и 25 закрыты: baseline зафиксирован, бренд runtime UI — `MyShop`, пользовательские UI-тексты и demo-data русскоязычные, CSS/JS/images находятся в tracked static, админка улучшена без замены стандартного Django Admin, аналитика вынесена в общий service layer, checkout переведён с always-success mock на weighted payment emulator.
...
- `ruff check` проходит;
- `pytest` проходит: `179 passed`, coverage `90%`;
...
2. Закрыть этап 26: email-уведомления.
3. После этого выбрать один крупный архитектурный трек: compatibility API, GraphQL, CI или production runtime.
