...
Актуальный baseline проекта на 2026-05-19 вынесен в `docs/current-state.md`. Если подробный чек-лист ниже расходится с этим снимком, приоритет для планирования имеет `docs/current-state.md`, а roadmap нужно синхронизировать перед началом нового крупного этапа.

К реализации уже сейчас готовы: завершение этапа 21 (UI, шаблоны, статика, русификация, адаптивность), этап 22 (расширенные русскоязычные demo-данные), UX-полировка web-флоу и финальная clean-проверка MVP по этапу 19. Этапы современной админки, аналитики, payment emulator, email, compatibility API, GraphQL, CI и production runtime лучше начинать с компактного ADR, потому что они меняют публичные контракты или инфраструктурные границы.
...
- [ ] Зафиксировать, что функционально уже реализованы каталог, карточка товара, корзина, checkout, личный кабинет и отзывы.
- [ ] Зафиксировать, что REST API, JWT и Swagger/OpenAPI уже подключены и покрыты тестами.
- [ ] Зафиксировать, что PostgreSQL и Docker Compose уже используются как базовая инфраструктура.
- [ ] Зафиксировать, что `seed_demo_data` уже существует, но нуждается в расширении и русификации.
- [ ] Зафиксировать, что Ruff, pytest и coverage настроены.
- [ ] Зафиксировать последнюю локальную проверку: `manage.py check` проходит.
- [ ] Зафиксировать последнюю локальную проверку: `ruff check` проходит.
- [ ] Зафиксировать последнюю локальную проверку: `pytest` проходит, `159 passed`, coverage `90%`.
- [ ] Отметить, что текущий roadmap местами оптимистичен: часть пунктов закрыта функционально, но не закрыта по презентации, production runtime и финальной сдаче.
- [ ] Составить отдельный список требований ТЗ, которые уже закрыты.
- [ ] Составить отдельный список требований ТЗ, которые остаются незакрытыми или требуют усиления.
- [x] Добавить сжатый индекс ADR в `docs/decisions/README.md`.
- [x] Актуализировать перегруженные ADR через раздел `Актуальная сжатая версия`.
- [ ] Перед реализацией новых этапов создать компактные ADR для конфликтов `C032`-`C039`.

Definition of Done этапа 20:

- [ ] В roadmap есть честный status snapshot.
- [ ] Отдельно перечислены реализованные требования ТЗ.
- [ ] Отдельно перечислены незакрытые требования ТЗ.
- [ ] Понятно, какие следующие этапы закрывают каждый оставшийся пробел.
...
- [ ] Использовать `src/prepare` как источник визуального концепта.
- [ ] Не использовать `src/prepare` как runtime-зависимость приложения.
- [ ] Зафиксировать конфликт `C033` перед переносом ассетов.
- [ ] Перенести нужные публичные CSS-файлы в tracked runtime-структуру `src/static/shop/css`.
- [ ] Перенести нужные публичные JS-файлы в tracked runtime-структуру `src/static/shop/js`.
- [ ] Перенести нужные публичные изображения в tracked runtime-структуру `src/static/shop/img`.
- [ ] Перенести статику админки в tracked runtime-структуру `src/static/admin_shop/`.
- [ ] Разнести Django-шаблоны по доменам: `catalog`, `cart`, `orders`, `users`, `admin`.
- [ ] Вынести общие повторяемые блоки в shared includes.
- [ ] Сохранить публичный бренд `MyShop`, несмотря на исходное название шаблона Hop & Barley.
- [ ] Зафиксировать конфликт `C032` перед заменой брендинга.
- [ ] Полностью русифицировать UI-тексты, title, navigation, кнопки, формы, пустые состояния и сообщения.
- [ ] Улучшить адаптивность ключевых страниц после переноса дизайна.
- [ ] Улучшить пустые состояния корзины, заказов, отзывов и каталога.
- [ ] Проверить, что `collectstatic` сможет собрать перенесённую runtime-статику.

Definition of Done этапа 21:

- [ ] В roadmap прописан перенос статики и шаблонов.
- [ ] Указано, что `src/prepare` остаётся reference/source.
- [ ] Указано, что публичное название проекта — только `MyShop`.
- [ ] Runtime-шаблоны не содержат ссылок на `src/prepare`.
- [ ] Все пользовательские тексты в runtime UI русскоязычные.

### Этап 22. Русскоязычные demo-данные

Статус: запланирован.

- [ ] Расширить `seed_demo_data` после документирования требований.
- [ ] Привести названия категорий к русскоязычному формату.
- [ ] Привести названия товаров к русскоязычному формату.
- [ ] Привести описания товаров к русскоязычному формату.
- [ ] Привести отзывы к русскоязычному формату.
- [ ] Привести имена покупателей к русскоязычному формату.
- [ ] Привести адреса доставки к русскоязычному формату.
- [ ] Привести комментарии к заказам к русскоязычному формату.
- [ ] Оставить технические ключи ASCII: `slug`, `SKU`, `username`, `provider_payment_id`.
- [ ] Добавить больше demo-заказов для аналитики.
- [ ] Добавить demo-заказы в статусе оплаченного заказа.
- [ ] Добавить demo-заказы в статусе ожидания оплаты.
- [ ] Добавить demo-заказы в статусе отмены.
- [ ] Добавить demo-заказы с неуспешной оплатой.
- [ ] Добавить demo-заказы в завершённом статусе.
- [ ] Добавить demo-платежи разных исходов для payment analytics.
- [ ] Подготовить demo-данные для admin dashboard.
- [ ] Подготовить demo-данные для GraphQL analytics.
- [ ] Связать demo-товары с runtime-изображениями после переноса статики.
- [ ] Сохранить идемпотентность команды.
- [ ] Сохранить защиту destructive reset через `--reset --yes`, `DEBUG=True` и local/demo ограничения.

Definition of Done этапа 22:

- [ ] Demo-данные выглядят как русскоязычный магазин.
- [ ] Seed остаётся идемпотентным.
- [ ] Reset остаётся защищённым.
- [ ] Demo-данных достаточно для проверки витрины, админской аналитики и GraphQL.
