...
- [x] `.venv\Scripts\python.exe -m pytest -q -p no:cacheprovider` — проходит, `101 passed`, покрытие `93%`.
...
- [x] Принят ADR 0015: merge реализуется сейчас, подключается к login-flow позже.
...
- [x] Подключение merge к login-flow отложено до этапа пользователей по ADR 0015.
- [x] `manage.py check`, `ruff check` и полный `pytest` проходят локально.
